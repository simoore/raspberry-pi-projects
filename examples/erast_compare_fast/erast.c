...
pthread_attr_t explicit_sched_attr;
pthread_attr_t orig_sched_attr;
struct sched_param explicit_param;
...
void set_scheduler(void)
{
    ...
    pthread_attr_init(&explicit_sched_attr);
    pthread_attr_setinheritsched(&explicit_sched_attr, PTHREAD_EXPLICIT_SCHED);
    pthread_attr_setschedpolicy(&explicit_sched_attr, SCHED_POLICY);

    max_prio=sched_get_priority_max(SCHED_POLICY);
    explicit_param.sched_priority=max_prio;    

    if((rc=sched_setscheduler(getpid(), SCHED_POLICY, &explicit_param)) < 0)
        perror("sched_setscheduler");

    pthread_attr_setschedparam(&explicit_sched_attr, &explicit_param);
...
void *invalidate_thread(void *threadptr)
{
    ...
    //printf("invalidate_thread %u FINAL j=%u\n", thargs.thread_idx, thargs.j);
...
int main(void)
{
	...
	printf("start test at %lf for %llu numbers\n", fnow-fstart, MAX);
        ...
        while( (p*p) <=  MAX)
        {
            ...
            for(thread_idx=0; thread_idx < NUM_THREADS; thread_idx++)
            {
                    ...
                    {
                        //printf("join thread %u FINAL j=%u\n", thread_idx, final_thread_j);
                    }
            }
	...
	clock_gettime(CLOCK_MONOTONIC, &now);
	fnow = (double)now.tv_sec  + (double)now.tv_nsec / 1000000000.0;
	printf("start test at %lf for %llu numbers with %u numbers per second\n", fnow-fstart, MAX, (unsigned int)(MAX / (fnow-fstart)));
