...
void *startService(void *threadid) {
    ...
    TIMEVAL_TO_TIMESPEC(&timeVal, &timeStartTest);
    ...
    rc = pthread_create(
        &threads[LOW_PRIO_SERVICE], &nrt_sched_attr, simpleTask, (void *)&threadParams[LOW_PRIO_SERVICE]);
    // rc = pthread_create(&threads[LOW_PRIO_SERVICE], &nrt_sched_attr, criticalSectionTask, (void
    // *)&threadParams[LOW_PRIO_SERVICE]);
    ...
    gettimeofday(&timeVal, NULL);
    TIMEVAL_TO_TIMESPEC(&timeVal, &timeNow);
    ...
    // rc = pthread_create(&threads[HIGH_PRIO_SERVICE], &rt_sched_attr, simpleTask, (void
    // *)&threadParams[HIGH_PRIO_SERVICE]);
    rc = pthread_create(
        &threads[HIGH_PRIO_SERVICE], &rt_sched_attr, criticalSectionTask, (void *)&threadParams[HIGH_PRIO_SERVICE]);
    ...
    if (runInterference > 0) {
        ...
        rc = pthread_create(
            &threads[MID_PRIO_SERVICE], &rt_sched_attr, simpleTask, (void *)&threadParams[MID_PRIO_SERVICE]);
...
void *simpleTask(void *threadp) {
    ...
    do {
        ...
        if (idleIdx == LOW_PRIO_SERVICE)
            printf("L%u ", idleCount[idleIdx]);
        else if (idleIdx == MID_PRIO_SERVICE)
            printf("M%u ", idleCount[idleIdx]);
        else if (idleIdx == HIGH_PRIO_SERVICE)
            printf("H%u ", idleCount[idleIdx]);
...
void *criticalSectionTask(void *threadp) {
    ...
    do {
        ...
        idleCount[idleIdx]++;
        if (idleIdx == LOW_PRIO_SERVICE)
            printf("CS-L%u ", idleCount[idleIdx]);
        else if (idleIdx == MID_PRIO_SERVICE)
            printf("CS-M%u ", idleCount[idleIdx]);
        else if (idleIdx == HIGH_PRIO_SERVICE)
            printf("CS-H%u ", idleCount[idleIdx]);
