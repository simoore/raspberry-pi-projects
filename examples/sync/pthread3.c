...
void *startService(void *threadid) {
    ...
    // Real-time option
    // rc = pthread_create(&threads[LOW_PRIO_SERVICE], &rt_sched_attr, criticalSectionTask, (void
    // *)&threadParams[LOW_PRIO_SERVICE]);

    // Non-real-time option
    rc = pthread_create(
        &threads[LOW_PRIO_SERVICE], &nrt_sched_attr, criticalSectionTask, (void *)&threadParams[LOW_PRIO_SERVICE]);
