...
static int process_image(const void *p, int size) {
    ...
    if (fmt.fmt.pix.pixelformat == V4L2_PIX_FMT_GREY) {
        ...
        for (i = 0, newi = 0; i < size; i = i + 4, newi = newi + 6) {
            ...
            yuv2rgb(y_temp, u_temp, v_temp, &scratchpad_buffer[newi], &scratchpad_buffer[newi + 1],
                &scratchpad_buffer[newi + 2]);
            yuv2rgb(y2_temp, u_temp, v_temp, &scratchpad_buffer[newi + 3], &scratchpad_buffer[newi + 4],
...
int seq_frame_read(void) {
    ...
    // buffers[frame_buf.index].start, frame_buf.bytesused); syslog(LOG_CRIT, "memcpy to %p from %p for %d bytes\n",
    // (void *)&(ring_buffer.save_frame[ring_buffer.tail_idx].frame[0]), buffers[frame_buf.index].start,
    // frame_buf.bytesused);
    memcpy((void *)&(ring_buffer.save_frame[ring_buffer.tail_idx].frame[0]), buffers[frame_buf.index].start,
        frame_buf.bytesused);
    ...
    if (read_framecnt > 0) {
        // printf("read_framecnt=%d, rb.tail=%d, rb.head=%d, rb.count=%d at %lf and %lf FPS", read_framecnt,
        // ring_buffer.tail_idx, ring_buffer.head_idx, ring_buffer.count, (fnow-fstart), (double)(read_framecnt) /
        // (fnow-fstart));

        // syslog(LOG_CRIT, "read_framecnt=%d, rb.tail=%d, rb.head=%d, rb.count=%d at %lf and %lf FPS", read_framecnt,
        // ring_buffer.tail_idx, ring_buffer.head_idx, ring_buffer.count, (fnow-fstart), (double)(read_framecnt) /
        // (fnow-fstart));
        syslog(LOG_CRIT, "read_framecnt=%d at %lf and %lf FPS", read_framecnt, (fnow - fstart),
            (double)(read_framecnt) / (fnow - fstart));
    } else {
        printf("at %lf\n", fnow);
    }
                ...
                if (nanosleep(&read_delay, &time_error) != 0)
                    ...
                    if (read_framecnt > 1) {
                        printf(" read at %lf, @ %lf FPS\n", (fnow - fstart),
                            (double)(read_framecnt + 1) / (fnow - fstart));

                        memcpy((void *)&(ring_buffer.save_frame[ring_buffer.tail_idx].frame[0]),
                            buffers[frame_buf.index].start, frame_buf.bytesused);
                        ...
                        process_image(
                            (void *)&(ring_buffer.save_frame[ring_buffer.head_idx].frame[0]), HRES * VRES * PIXEL_SIZE);
                        ...
                        printf("process from rb.tail=%d, rb.head=%d, ptr=%p\n", ring_buffer.tail_idx,
                            ring_buffer.head_idx, (void *)&(ring_buffer.save_frame[ring_buffer.head_idx].frame[0]));
...
static void init_mmap(char *dev_name) {
    ...
    for (n_buffers = 0; n_buffers < req.count; ++n_buffers) {
        ...
        buffers[n_buffers].start = mmap(NULL /* start anywhere */, frame_buf.length,
            PROT_READ | PROT_WRITE /* required */, MAP_SHARED /* recommended */, camera_device_fd, frame_buf.m.offset);
